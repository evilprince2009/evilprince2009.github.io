...
export class ShellComponent implements IComponent {
  ...
  render(): void {
    const shellTemplate: HTMLDivElement = document.createElement(
      "div"
    ) as HTMLDivElement;
    shellTemplate.classList.add("shell");

    const shellTitle: HTMLLabelElement = document.createElement(
      "label"
    ) as HTMLLabelElement;
  ...
  shellFocus(): void {
    const target: NodeListOf<HTMLInputElement> = document.querySelectorAll(
      ".command"
    );
    const lastInput: HTMLInputElement =
      target[document.querySelectorAll(".command").length - 1];
    lastInput.focus();
  }
}
